...
pub const REQUIRED_UNITS: [&str; 3] = ["dbus.service", "NetworkManager.service", "lightdm.service"];
...
impl ReadinessState {
    ...
    fn on_journal(&mut self, line: &JournalLine) -> Vec<ReadinessEvent> {
        ...
        if self.session_ns.is_none() && from_lightdm && line.message.contains(&self.session_needle)
        {
